...
export default function VehicleDispatch() {
  ...
  const [selectedActiveDispatchTimeline, setSelectedActiveDispatchTimeline] =
    useState(null);
  const [selectedMapVehiclePopover, setSelectedMapVehiclePopover] =
    useState(null);
  ...
  const matchedAvailableVehicles = allFleetVehicles.filter(
    (v) =>
      v.type === selectedTask.requiredVehicleType && v.status === "AVAILABLE",
  );
  ...
  const paginatedPendingTasks = filteredTasks.slice(
    (tasksPage - 1) * tasksPerPage,
    tasksPage * tasksPerPage,
  );

  const totalActiveDspPages = Math.max(
    1,
    Math.ceil(activeDispatches.length / activeDspPerPage),
  );
  const paginatedActiveDispatches = activeDispatches.slice(
    (activeDspPage - 1) * activeDspPerPage,
    activeDspPage * activeDspPerPage,
  );
  ...
  const handleExecuteDispatch = () => {
    ...
    const generatedGateBookingId =
      selectedTask.taskType !== "YARD_MOVE"
    ...
    try {
      yardTaskService
        .createTask({
          ...
          blockCode: "A01",
          operationType:
            selectedTask.taskType === "PORT_PICKUP"
              ? "Load"
              : selectedTask.taskType === "PORT_DELIVERY"
                ? "Unload"
                : "Relocate",
        ...
        .catch((err) =>
          console.warn("Could not sync to yard tasks backend:", err),
        );
    } catch (e) {
    ...
    setToastMessage(
      `⚡ Đã tạo lệnh điều độ ${newDspId} và tự động gửi Gate Booking ${generatedGateBookingId} thành công!`,
    );
  ...
  const handleConfirmReassign = (backupVehicle) => {
    ...
    setToastMessage(
      `🔄 Đã tái chỉ định xe ${backupVehicle.id} thay thế cho lệnh bị trễ thành công!`,
    );
  ...
  const handleCreateTask = (e) => {
    ...
    const createdTask = {
      ...
      requiredVehicleName:
        newTaskForm.requiredVehicleType === "ROAD_TRUCK"
          ? "🚚 Xe Đầu Kéo Đường Dài"
          : "🚜 Xe Nội Bãi (Yard Tractor)",
      waitingTime: "Vừa tạo",
      status: "Pending",
    };
    ...
    setToastMessage(
      `✅ Đã tạo nhiệm vụ điều phối mới: ${createdTask.taskTypeName} (${createdTask.container})!`,
    );
    setTimeout(() => setToastMessage(""), 3500);
  };
    ...
    <div className="p-8 w-full font-sans flex flex-col gap-6 relative">
      ...
      <div className="flex justify-between items-center bg-white border border-chalk rounded-2xl p-5 shadow-sm">
        <div>
          ...
          <h2 className="font-heading text-3xl text-carbon font-extrabold mt-1">
            Điều Phối Phương Tiện
          </h2>
          <p className="text-xs text-slate mt-0.5">
            Điều phối xe nhận/giao container về Cảng Tiên Sa & xe nội bãi, theo
            dõi quá trình realtime.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="bg-white border border-chalk rounded-xl p-4 shadow-sm space-y-1">
          <span className="text-slate text-[10px] uppercase font-bold tracking-wider">
            Đang Chờ Gán Xe
          </span>
          <div className="text-3xl font-extrabold text-amber-500 font-mono">
            {kpiStats.pending}
          </div>
          <span className="text-[11px] text-slate font-bold">
            Nhiệm vụ chờ gán xe
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-3 bg-white border border-chalk rounded-2xl p-5 shadow-sm space-y-4">
          <div className="border-b border-chalk pb-3">
            <span className="text-[10px] font-bold text-signal-orange uppercase tracking-wider block">
              HÀNG ĐỢI NHIỆM VỤ
            </span>
            <h3 className="font-heading text-lg font-extrabold text-carbon">
              Yêu Cầu Điều Phối
            </h3>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5 text-[11px] font-bold">
            {["All", "PORT_PICKUP", "PORT_DELIVERY", "YARD_MOVE", "High"].map(
              (f) => (
                <button
                  key={f}
            ...
            {filteredTasks.map((task) => (
              ...
              >
                <div className="flex justify-between items-start mb-1.5">
                  <span
                    className={`px-2 py-0.5 rounded text-[9px] ${task.priorityClass}`}
                  >
                    ● {task.priority} PRIORITY
                  </span>
                  <span className="text-[10px] text-slate font-sans">
                    Chờ: {task.waitingTime}
                  </span>
                </div>

                <h4 className="font-heading font-extrabold text-sm text-carbon">
                  {task.taskTypeName}
                </h4>
                <div className="text-signal-orange font-bold text-xs mt-0.5">
                  {task.container} ({task.containerType})
                </div>

                <div className="bg-fog p-2.5 rounded-lg border border-chalk my-2 text-[11px] font-sans space-y-1">
                  <div>
                    Từ:{" "}
                    <strong className="text-carbon font-mono">
                      {task.origin}
                    </strong>
                  </div>
                  <div>
          ...
          {totalTasksPages > 1 && (
            <div className="flex items-center justify-between pt-2 border-t border-chalk text-xs">
              <span className="text-[11px] text-slate font-sans">
                Trang <strong>{tasksPage}</strong> / {totalTasksPages} (
                {filteredTasks.length} yêu cầu)
              </span>
        ...
        <div className="lg:col-span-6 bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4 relative min-h-[560px] overflow-hidden flex flex-col justify-between">
          ...
          >
            ...
            <rect
              width="100%"
              height="100%"
              fill="url(#dispatchBlueprintGrid)"
            />
            <line
              x1="0"
              y1="160"
          ...
          <div className="grid grid-cols-3 gap-4 z-10 font-mono text-xs my-2">
            ...
            <div className="bg-red-50 p-3 rounded-xl border-2 border-red-400 shadow-sm text-center">
              ...
              <span className="text-[10px] text-red-700 font-sans font-bold">
                188 Cont • RTG-02 (94%)
              </span>
            </div>
            <div className="bg-white/90 p-3 rounded-xl border border-slate-300 shadow-sm text-center">
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-2 z-10">
            <span className="text-[10px] font-bold text-slate-500 uppercase block font-sans">
              VỊ TRÍ PHƯƠNG TIỆN REALTIME TRÊN BẢN ĐỒ:
            </span>
            <div className="grid grid-cols-2 gap-2 text-xs font-mono">
              {allFleetVehicles.map((v) => (
                ...
                >
                  <div>
                    <strong className="text-carbon block">
                      {v.id} ({v.plate})
                    </strong>
                    <span className="text-[10px] text-slate-500 font-sans">
                      {v.driver}
          ...
          <div className="bg-carbon text-white p-3 rounded-xl text-xs font-mono flex justify-between items-center z-10 shadow-lg">
            <span>
              NHIỆM VỤ ĐANG CHỌN:{" "}
              <strong className="text-signal-orange">{selectedTask.id}</strong>{" "}
              ({selectedTask.taskTypeName})
            </span>
            <span className="text-[11px] text-gray-300">
              Yêu cầu xe: {selectedTask.requiredVehicleType}
            </span>
          </div>
        </div>
        ...
        <div className="lg:col-span-3 bg-white border border-chalk rounded-2xl p-5 shadow-sm space-y-4">
          ...
          <div className="bg-fog p-4 rounded-xl border border-chalk space-y-2 text-xs font-mono">
            <div className="flex justify-between">
              <span className="text-slate font-sans">Mã Nhiệm vụ:</span>
              <strong className="text-carbon">{selectedTask.id}</strong>
            </div>
            <div className="flex justify-between">
              <span className="text-slate font-sans">Loại lệnh:</span>
              <strong className="text-signal-orange font-sans font-bold">
                {selectedTask.taskTypeName}
              </strong>
            </div>
            <div className="flex justify-between">
              <span className="text-slate font-sans">Mã Container:</span>
              <strong className="text-carbon font-bold">
                {selectedTask.container}
              </strong>
            </div>
            <div className="flex justify-between">
              <span className="text-slate font-sans">Điểm xuất phát:</span>
              <strong className="text-carbon font-sans">
                {selectedTask.origin}
              </strong>
            </div>
            <div className="flex justify-between">
              <span className="text-slate font-sans">Điểm đến:</span>
              <strong className="text-signal-orange font-sans font-bold">
                {selectedTask.destination}
              </strong>
            </div>
            <div className="flex justify-between border-t border-chalk pt-2">
              <span className="text-slate font-sans">Xe bắt buộc:</span>
              <strong className="text-carbon font-bold">
                {selectedTask.requiredVehicleName}
              </strong>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <span className="text-[10px] font-bold text-slate uppercase block font-sans">
              DANH SÁCH XE SẴN SÀNG PHÙ HỢP ({matchedAvailableVehicles.length})
            </span>
            ...
            {matchedAvailableVehicles.length === 0 ? (
              <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-red-900 text-xs font-sans space-y-1">
                <strong>
                  ⚠ Không có xe {selectedTask.requiredVehicleType} nào sẵn sàng!
                </strong>
                <p className="text-[11px]">
                  Tất cả các xe loại này đều đang di chuyển hoặc bảo trì.
                </p>
              </div>
            ) : (
              <div className="space-y-2 max-h-48 overflow-y-auto font-mono text-xs">
                {matchedAvailableVehicles.map((veh) => (
                  <div
                    key={veh.id}
                    onClick={() =>
                      setSelectedVehicleForTask({
                        ...veh,
                        selectedDriver: veh.driver,
                      })
                    }
                  ...
                  >
                    <div>
                      ...
                      <span className="text-[10px] text-slate font-sans">
                        Tài xế mặc định: {veh.driver} • ETA: {veh.eta}
                      </span>
                    </div>

                    <span
                      className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${selectedVehicleForTask?.id === veh.id ? "bg-signal-orange text-white" : "bg-green-600 text-white"}`}
                    >
                      {selectedVehicleForTask?.id === veh.id
                        ? "✓ Đã chọn"
                        : "Chọn xe"}
          ...
          {selectedVehicleForTask && (
            <div className="p-3.5 bg-orange-50 border border-orange-200 rounded-xl space-y-2 text-xs font-mono">
              ...
              <select
                value={
                  selectedVehicleForTask.selectedDriver ||
                  selectedVehicleForTask.driver
                }
                onChange={(e) =>
                  setSelectedVehicleForTask({
                    ...selectedVehicleForTask,
              ...
              >
                <option value="Nguyễn Văn A">
                  Nguyễn Văn A (Bằng FC - Kinh nghiệm 8 năm)
                </option>
                <option value="Trần Văn B">
                  Trần Văn B (Bằng FC - Kinh nghiệm 5 năm)
                </option>
                <option value="Phạm Văn D">
                  Phạm Văn D (Bằng FC - Kinh nghiệm 6 năm)
                </option>
                <option value="Lê Văn C">
                  Lê Văn C (Bằng FE - Chuyên xe Lạnh)
                </option>
                <option value="Hoàng Văn E">
                  Hoàng Văn E (Bằng FC - Kinh nghiệm 4 năm)
                </option>
              </select>
              <span className="text-[10px] text-slate font-sans block">
                Dispatcher có thể giữ tài xế mặc định hoặc chọn tài xế khác nhận
                lệnh này.
              </span>
            </div>
          )}
          ...
          <button
            ...
            className={`w-full h-12 rounded-full font-extrabold text-xs transition-opacity shadow-lg flex items-center justify-center gap-2 ${
              selectedVehicleForTask
                ? "bg-signal-orange text-white hover:opacity-95"
                : "bg-chalk text-slate cursor-not-allowed"
            }`}
          >
        ...
        <div className="flex justify-between items-center border-b border-chalk pb-3">
          <div>
            <h3 className="font-heading text-xl font-extrabold text-carbon">
              Nhiệm Vụ Đang Thực Hiện (Active Dispatches)
            </h3>
            <p className="text-xs text-slate font-mono mt-0.5">
              Theo dõi quá trình vận chuyển container thời gian thực
            </p>
          </div>
          <span className="text-xs font-bold text-carbon font-mono">
            {activeDispatches.length} Lệnh đang chạy
          </span>
        </div>
          ...
          {activeDispatches.map((dsp) => (
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[10px] font-bold text-slate block">
                    {dsp.dspId}
                  </span>
                  <strong className="text-base text-carbon">
                    {dsp.vehicle} ({dsp.plate})
                  </strong>
                </div>

                <div className="bg-white p-3 rounded-xl border border-chalk space-y-1 font-sans">
                  <div className="font-bold text-carbon">
                    Container: {dsp.container}
                  </div>
                  <div className="text-slate text-[11px]">
                    Từ: <strong>{dsp.origin}</strong> ➔ Đến:{" "}
                    <strong className="text-signal-orange">
                      {dsp.destination}
                    </strong>
                  </div>
                  <div className="text-[10px] text-slate font-mono pt-1">
                    Gate Booking: <strong>{dsp.gateBookingId}</strong>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center pt-1 font-sans">
                  ...
                  {dsp.status === "DELAYED" ? (
                    ...
                    <span className="text-signal-orange font-bold text-[11px]">
                      Xem Timeline ➔
                    </span>
                  )}
                </div>
        ...
        {totalActiveDspPages > 1 && (
          <div className="flex items-center justify-between pt-3 border-t border-chalk text-xs">
            <span className="text-slate font-sans">
              Hiển thị trang <strong>{activeDspPage}</strong> trên tổng số{" "}
              <strong>{totalActiveDspPages}</strong> trang (
            ...
            <div className="flex items-center gap-1.5 font-sans">
              ...
              <button
                onClick={() =>
                  setActiveDspPage((p) => Math.min(totalActiveDspPages, p + 1))
                }
        ...
        <div className="fixed inset-0 bg-carbon/80 backdrop-blur-sm z-50 flex items-center justify-center p-6">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full space-y-6 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="bg-fog p-5 rounded-2xl border border-chalk space-y-3 font-mono text-xs">
              <div className="flex justify-between">
                <span className="text-slate font-sans">Nhiệm vụ:</span>
                <strong className="text-carbon font-bold">
                  {selectedTask.taskTypeName}
                </strong>
              </div>
              ...
              <div className="flex justify-between">
                <span className="text-slate font-sans">Phương tiện chọn:</span>
                <strong className="text-signal-orange font-bold">
                  {selectedVehicleForTask.id} ({selectedVehicleForTask.plate})
                </strong>
              </div>
              <div className="flex justify-between">
                <span className="text-slate font-sans">Tài xế phụ trách:</span>
                <strong className="text-carbon font-bold">
                  {selectedVehicleForTask.driver}
                </strong>
              </div>
              ...
              <div className="flex justify-between border-t border-chalk pt-2">
                <span className="text-slate font-sans font-bold text-green-700">
                  Tự động liên kết:
                </span>
                <strong className="text-green-700 font-bold">
                  Gate Booking (Kèm theo)
                </strong>
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl p-8 max-w-md w-full space-y-6 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="p-4 bg-red-50 border border-red-200 rounded-2xl text-xs font-mono space-y-1">
              <strong className="text-red-900 font-bold block">
                🔴 LỆNH ĐANG TRỄ HẠN: TRK-008 (MSCU9876543)
              </strong>
              <p className="text-[11px] text-red-800 font-sans">
                Lý do: Kẹt thủ tục tại Cổng 01. Vui lòng chọn xe thay thế ngay
                để giải phóng container.
              </p>
            </div>

            <div className="space-y-2 font-mono text-xs">
              <span className="text-[10px] font-bold text-slate uppercase block font-sans">
                DANH SÁCH XE ROAD TRUCK SẴN SÀNG THAY THẾ:
              </span>
              {allFleetVehicles
                .filter(
                  (v) => v.type === "ROAD_TRUCK" && v.status === "AVAILABLE",
                )
                .map((veh) => (
                  <div
                    key={veh.id}
                    className="p-3 bg-fog rounded-xl border border-chalk flex justify-between items-center"
                  >
                    <div>
                      <strong className="text-carbon block">
                        {veh.id} ({veh.plate})
          ...
          <div className="bg-white rounded-3xl p-8 max-w-md w-full space-y-6 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center border-b border-chalk pb-4">
              <div>
                ...
                <h3 className="font-heading text-2xl font-extrabold text-carbon">
                  {selectedActiveDispatchTimeline.dspId}
                </h3>
                <span className="text-xs text-slate font-mono block mt-0.5">
                  {selectedActiveDispatchTimeline.vehicle} •{" "}
                  {selectedActiveDispatchTimeline.container}
                </span>
              </div>
              <button
            ...
            <div className="space-y-3 font-mono text-xs pl-2 border-l-2 border-chalk">
              <div className="flex items-center gap-3">
                ...
                <span className="text-carbon font-bold">
                  1. Đã khởi tạo nhiệm vụ (Task Created)
                </span>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-carbon font-bold">
                  2. Đã chỉ định xe ({selectedActiveDispatchTimeline.vehicle})
                </span>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-carbon font-bold">
                  3. Xác nhận điều động (Dispatch Confirmed)
                </span>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-carbon font-bold">
                  4. Tự động tạo Gate Booking (
                  {selectedActiveDispatchTimeline.gateBookingId})
                </span>
              </div>

              <div className="flex items-center gap-3">
                <span
                  className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${selectedActiveDispatchTimeline.timelineStep >= 5 ? "bg-purple-600 text-white animate-pulse" : "bg-chalk text-slate"}`}
                >
                  ●
                </span>
                <span
                  className={`font-bold ${selectedActiveDispatchTimeline.timelineStep >= 5 ? "text-purple-600" : "text-slate"}`}
                >
                  5. Xe đang di chuyển lưu thông
                </span>
              </div>

              <div className="flex items-center gap-3">
                <span className="w-5 h-5 rounded-full bg-chalk text-slate flex items-center justify-center text-[10px] font-bold">
                  ○
                </span>
          ...
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-lg w-full space-y-5 shadow-2xl animate-in zoom-in-95 duration-200 font-sans">
            ...
            >
              <div>
                ...
                >
                  <option value="PORT_DELIVERY">
                    🚛 Giao Container Về Cảng (Port Delivery - Kho ➔ Cảng)
                  </option>
                  <option value="PORT_PICKUP">
                    🚚 Lấy Container Tại Cảng (Port Pickup - Cảng ➔ Kho)
                  </option>
                  <option value="YARD_MOVE">
                    🚜 Chuyển Container Nội Bãi (Yard Move - Bãi A ➔ Bãi B)
                  </option>
                </select>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewTaskForm({ ...newTaskForm, origin: e.target.value })
                  }
                  required
