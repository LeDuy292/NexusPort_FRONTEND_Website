...
export default function ContainerManagement() {
  ...
  return <div className="mx-auto w-full max-w-[1500px] space-y-5 p-1">
    <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-signal-orange"><span className="h-2 w-2 rounded-full bg-signal-orange" />Central Registry · Đăng kiểm trung tâm</div><h1 className="font-heading text-3xl font-bold text-carbon">Quản lý Container</h1><p className="mt-1 text-sm text-slate">Theo dõi xuyên suốt Booking → Gate-In → Yard → Gate-Out</p></div>{canWrite && <button onClick={() => { setFormContainer(undefined); setShowForm(true) }} className="flex items-center justify-center gap-2 rounded-lg bg-signal-orange px-5 py-3 text-sm font-bold text-white shadow-sm hover:bg-orange-600"><span className="material-symbols-outlined text-lg">add</span>Đăng ký Container</button>}</div>
