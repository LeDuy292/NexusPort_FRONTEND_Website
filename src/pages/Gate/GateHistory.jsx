...
export default function GateHistory() {
  ...
  const loadHistory = async () => {
    ...
    if (res.success && Array.isArray(res.data)) {
      // Map API fields sang UI format
      const mapped = res.data.map((item, idx) => ({
        id: item.id || `REC-${idx + 1}`,
        time: item.checkedAtUtc ? new Date(item.checkedAtUtc).toLocaleTimeString('vi-VN') : '—',
        vehicle: item.vehicleType || 'Xe Container',
        plate: item.licensePlate || '—',
        driver: item.driverName || '—',
        container: item.containerNumber || '—',
        gate: item.gateLaneId || 'Gate A',
        action: item.operationType === 'GateOut' ? 'CHECK-OUT' : 'CHECK-IN',
        result: item.status === 'PASS' ? 'Passed' : 'Rejected',
    ...
    <div className="p-6 md:p-8 w-full font-sans flex flex-col gap-6 bg-slate-50 min-h-screen">
      ...
      <div className="bg-white border border-chalk rounded-2xl p-5 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate mt-0.5">
            Toàn bộ lịch sử kiểm soát xe ra vào cổng cảng từ Backend API.
          </p>
        </div>
        <button
          onClick={loadHistory}
          className="h-10 px-4 border border-chalk text-slate rounded-xl font-extrabold text-xs hover:bg-fog flex items-center gap-1.5 cursor-pointer"
        >
      ...
      <div className="bg-white border border-chalk rounded-2xl p-4 shadow-sm flex flex-col md:flex-row gap-3 items-center">
        <div className="relative w-full max-w-xs">
          <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate text-sm">
            search
          </span>
          <input
            type="text"
      ...
      <div className="bg-white rounded-2xl border border-chalk shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          {loading ? (
            <div className="py-12 text-center text-slate font-bold text-sm">
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-chalk">
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan="10" className="py-12 text-center text-slate font-bold text-sm">
                  ...
                  filtered.map((log) => (
                    <tr key={log.id} className="hover:bg-fog/70">
                      ...
                      <td className="py-3 px-4 font-mono text-signal-orange text-[11px]">
                        {log.bookingId || '—'}
                      </td>
                      <td className="py-3 px-4 text-right">
                        <button
                          onClick={() => setSelectedLog(log)}
                          className="px-3 py-1.5 bg-carbon text-white rounded-lg font-extrabold text-[11px] hover:bg-black flex items-center gap-1 ml-auto cursor-pointer"
                        >
        ...
        <div className="fixed inset-0 bg-carbon/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md max-h-[92vh] rounded-3xl p-7 shadow-2xl overflow-y-auto animate-in zoom-in-95 space-y-4">
            <div className="flex justify-between items-center border-b border-chalk pb-3">
              <div>
                <span className="text-[10px] font-extrabold text-signal-orange uppercase font-mono block">
                  GATE LOG DETAIL
                </span>
                <h3 className="font-heading text-lg font-extrabold text-carbon font-mono">
                  {selectedLog.id}
                </h3>
              </div>
              <button
                onClick={() => setSelectedLog(null)}
                className="w-8 h-8 rounded-full bg-fog border border-chalk flex items-center justify-center cursor-pointer"
              >
            ...
            <div className="grid grid-cols-2 gap-2 text-xs font-mono">
              {[
                ...
                ['Gate', selectedLog.gate],
                ['Vehicle', selectedLog.vehicle],
                ['Biển số', selectedLog.plate],
                ['Driver', selectedLog.driver],
            ...
            <button
              onClick={() => setSelectedLog(null)}
              className="w-full h-10 bg-carbon text-white rounded-xl font-extrabold text-xs cursor-pointer"
            >
              Đóng
            </button>
          </div>
